...
readonly source_name="gao-openfhe-a2b-matched.cpp"
...
if [[ ! -d "${build_dir}/src/pke/CMakeFiles/gao-openfhe-a2b-matched.dir" ]]; then
    cmake -S "${source_root}" -B "${build_dir}"
fi
cmake --build "${build_dir}" --target gao-openfhe-a2b-matched --parallel "${GAO_BUILD_JOBS:-$(nproc)}"
printf '%s\n' "${build_dir}/bin/examples/pke/gao-openfhe-a2b-matched"
