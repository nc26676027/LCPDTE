...
const RouteBSigned8SameFeatureBinaryAccuracyTolerance = 5e-4

type RouteBCanonicalSigned8SameFeatureBinaryResult struct {
	...
	SameFeatureBinary RouteBSigned8SameFeatureBinaryReport `json:"same_feature_binary"`
	...
	PostBinaryPeakRSSBytes   uint64    `json:"post_binary_peak_rss_bytes"`
...
func (result RouteBCanonicalSigned8SameFeatureBinaryResult) Validate() error {
	...
	if err := result.SameFeatureBinary.Validate(); err != nil {
		return err
	}
	for operation, evidence := range map[string]RuntimeCapacityEvidenceReport{
		...
		if err := evidence.Validate(); err != nil || evidence.Operation != operation {
			return lineagef("canonical signed8 same-feature binary runtime evidence %q changed", operation)
		}
	}
	capacityIdentity, err := decodeRBAUTHHexDigest("Route-B signed8 same-feature binary capacity plan", result.SameFeatureBinary.Base.CapacityPlanDigest)
	if err != nil {
		return err
	}
	if result.FirstOperation.RuntimeCapacity.Operation != routeBRuntimeOperationSigned8Depth2NodeBatch ||
		...
		result.PostBinaryPeakRSSBytes < result.PostInstallPeakRSSBytes || result.TotalWallNanoseconds == 0 {
		return lineagef("canonical signed8 same-feature binary linkage, construction counter, RSS, or wall-time changed")
	}
	if result.QueryCount != routeBSigned8Radix4Queries || result.InputWords != routeBSigned8Radix4Words ||
		...
		len(result.DecodedOutput) != routeBSigned8Radix4Slots || result.AccuracyTolerance != RouteBSigned8SameFeatureBinaryAccuracyTolerance {
		return lineagef("canonical signed8 same-feature binary input, output, or tolerance shape changed")
	}
	if result.ApplicationSecurityProfileDigest != routeBApplicationSecurityProfileDigest ||
		...
		return lineagef("canonical signed8 same-feature binary security binding or no-decryption ledger changed")
	}
	...
	binaryRanges, binaryModel, err := reconstructRouteBSigned8Depth2NodeBatchInputs(
		result.SameFeatureBinary.Base.Ranges, result.SameFeatureBinary.Base.Model,
	)
	if err != nil {
		return err
	}
	model, ranges, expectedBinaryModel, expectedBinaryRanges, err := routeBSigned8Radix4CanonicalModelsAndRanges()
	if err != nil {
		return err
	}
	if binaryModel.Digest() != expectedBinaryModel.Digest() {
		return lineagef("canonical signed8 same-feature binary model changed")
	}
	for index := range binaryRanges {
		if binaryRanges[index].Digest() != expectedBinaryRanges[index].Digest() {
			return lineagef("canonical signed8 same-feature binary range %d changed", index)
		}
	}
	...
	for query, feature := range values {
		...
		for slot := 0; slot < 4; slot++ {
			...
			if !routeBFiniteFloat(got.Real) || !routeBFiniteFloat(got.Imag) {
				return lineagef("canonical signed8 same-feature binary active slot %d is non-finite", index)
			}
	...
	for word := 0; word < routeBSigned8Radix4Words; word++ {
		...
		for slot := 0; slot < 4; slot++ {
			...
			got := result.DecodedOutput[index]
			if !routeBFiniteFloat(got.Real) || !routeBFiniteFloat(got.Imag) {
				return lineagef("canonical signed8 same-feature binary inactive slot %d is non-finite", index)
			}
	...
	if pathCounts != [4]uint32{55, 29, 30, 56} || equalityCounts != [3]uint32{2, 2, 2} {
		return lineagef("canonical signed8 same-feature binary coverage changed: paths=%v equality=%v", pathCounts, equalityCounts)
	}
	if result.MaxActiveAbsError != maxActive || result.MaxInactiveAbs != maxInactive || result.MaxImaginaryAbs != maxImaginary ||
		...
		return lineagef("canonical signed8 same-feature binary decoded accuracy/equivalence ledger changed or failed")
	}
	return nil
}

func RunCanonicalRouteBSigned8SameFeatureBinary() (result RouteBCanonicalSigned8SameFeatureBinaryResult, err error) {
	started := time.Now()
	before := dft.SnapshotMatrixConstructionCounters()
	authority, err := NewRouteBAuthority()
	if err != nil {
		return result, fmt.Errorf("secureeval: canonical signed8 same-feature binary create Authority: %w", err)
	}
	releaseRouteBTransientHeap()
	buildSpec, buildPermit, err := authority.AuthorizeBuild()
	if err != nil {
		return result, fmt.Errorf("secureeval: canonical signed8 same-feature binary AuthorizeBuild: %w", err)
	}
	releaseRouteBTransientHeap()
	receipt, artifact, err := authority.BeginBuild(buildPermit)
	if err != nil {
		return result, fmt.Errorf("secureeval: canonical signed8 same-feature binary BeginBuild: %w", err)
	}
	releaseRouteBTransientHeap()
	readySpec, readyPermit, err := authority.AuthorizeReady(buildPermit, receipt, artifact)
	if err != nil {
		return result, fmt.Errorf("secureeval: canonical signed8 same-feature binary AuthorizeReady: %w", err)
	}
	releaseRouteBTransientHeap()
	...
	keyGenerator = nil
	if secretKey == nil {
		return result, lineagef("canonical signed8 same-feature binary secret-key generation returned nil")
	}
	...
	for _, value := range inputWords {
		...
		if blockErr != nil {
			return result, fmt.Errorf("secureeval: encode canonical signed8 same-feature binary input: %w", blockErr)
		}
		inputSlots = append(inputSlots, block...)
	}
	if len(inputSlots) != routeBSigned8Radix4Slots {
		return result, lineagef("canonical signed8 same-feature binary input slot count changed")
	}
	...
	if err = encoder.Encode(inputSlots, plaintext); err != nil {
		return result, fmt.Errorf("secureeval: encode canonical signed8 same-feature binary input plaintext: %w", err)
	}
	encryptor := ckks.NewEncryptor(parameters, secretKey)
	input, err := encryptor.EncryptNew(plaintext)
	if err != nil {
		return result, fmt.Errorf("secureeval: encrypt canonical signed8 same-feature binary input: %w", err)
	}
	...
	if err != nil {
		return result, fmt.Errorf("secureeval: canonical signed8 same-feature binary Install: %w", err)
	}
	...
	binaryResult, firstOperation, binaryReport, err := installed.RunSigned8SameFeatureBinaryControl(input)
	input = nil
	if err != nil {
		return result, fmt.Errorf("secureeval: canonical Route-B signed8 same-feature binary: %w", err)
	}
	output := binaryResult.Ciphertext()
	if output == nil || binaryResult.ReportDigest() != binaryReport.Digest {
		return result, lineagef("canonical signed8 same-feature binary output is nil or has foreign provenance")
	}
	outputCiphertextBytes := uint64(output.BinarySize())
	postBinaryPeak, err := sampleProductionProcessPeakRSS()
	if err != nil {
		return result, err
	}
	...
	if err = decoder.Decode(outputPlaintext, values); err != nil {
		return result, fmt.Errorf("secureeval: decode canonical signed8 same-feature binary output: %w", err)
	}
	outputPlaintext = nil
	decoder = nil
	decryptor = nil
	...
	for query, feature := range inputValues {
		...
		for slot := 0; slot < 4; slot++ {
			...
			if errorMagnitude > RouteBSigned8SameFeatureBinaryAccuracyTolerance || math.Abs(value.Imag) > RouteBSigned8SameFeatureBinaryAccuracyTolerance {
				activeMismatches++
			}
		}
	}
	for word := 0; word < routeBSigned8Radix4Words; word++ {
		...
		for slot := 0; slot < 4; slot++ {
			...
			maxImaginary = math.Max(maxImaginary, math.Abs(value.Imag))
			if math.Abs(value.Real) > RouteBSigned8SameFeatureBinaryAccuracyTolerance || math.Abs(value.Imag) > RouteBSigned8SameFeatureBinaryAccuracyTolerance {
				inactiveMismatches++
			}
		}
	}
	...
	result = RouteBCanonicalSigned8SameFeatureBinaryResult{
		BuildSpec: buildSpec, BuildReceipt: receipt.Report(), ReadySpec: readySpec,
		FirstOperation: firstOperation, SameFeatureBinary: binaryReport,
		...
		PostInstallPeakRSSBytes: postInstallPeak, PostBinaryPeakRSSBytes: postBinaryPeak,
		...
		DecodedOutput: decoded, AccuracyTolerance: RouteBSigned8SameFeatureBinaryAccuracyTolerance,
	...
	if err = result.Validate(); err != nil {
		return RouteBCanonicalSigned8SameFeatureBinaryResult{}, err
	}
	return result, nil
}
